...
authCommand
  ...
  .addHelpText("after", "\nExample:\n  typefully-cli auth set sk-abc123xyz")
  .action((token: string) => {
    setToken(token);
...
authCommand
  ...
  .addHelpText("after", "\nExample:\n  typefully-cli auth show\n  typefully-cli auth show --raw")
  .action((opts: { raw?: boolean }) => {
    if (!hasToken()) {
      log.warn("No token configured. Run: typefully-cli auth set <token>");
      return;
    }
